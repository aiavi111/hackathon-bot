...
async def team_size_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if team_size < 2 or team_size > 6:
        await callback.answer("Можно выбрать только от 2 до 6 участников.")
        return
...
async def send_receipt_to_group(message: Message, state: FSMContext, is_photo: bool):
    if not GROUP_CHAT_ID:
        await message.answer("Ошибка: GROUP_CHAT_ID не найден в .env")
        return
...
async def approve_by_reply(message: Message):
    ...
    await message.answer("✅ Команда подтверждена. Участнику отправлено сообщение, состав добавлен в список.")